...
async function initDB() {
  ...
  try { await db.query('ALTER TABLE orders ADD COLUMN return_proof LONGTEXT'); } catch(_) {}
...
app.delete('/api/admin/users/:id', requireAdmin, async (req, res) => {
  try {
    ...
    await db.query('DELETE FROM order_items WHERE order_num IN (SELECT order_num FROM orders WHERE user_id=?)', [id]);
    await db.query('DELETE FROM orders WHERE user_id=?', [id]);
    await db.query('DELETE FROM saved_addresses WHERE user_id=?', [id]);
...
app.get('/api/admin/orders', requireAdmin, async (req, res) => {
  try {
    const [orders] = await db.query(
      'SELECT o.*, u.username AS username FROM orders o JOIN users u ON o.user_id=u.id ORDER BY o.created_at DESC'
    );
...
app.put('/api/orders/:orderNum/resubmit-gcash', requireAuth, async (req, res) => {
  const { gcash_proof, gcash_ref, gcash_status } = req.body;
  try {
    const [rows] = await db.query('SELECT * FROM orders WHERE order_num=? AND user_id=?', [req.params.orderNum, req.session.userId]);
    if (!rows.length) return res.status(404).json({ error: 'Order not found.' });
    if (rows[0].gcash_status !== 'rejected') return res.status(400).json({ error: 'Only rejected orders can be resubmitted.' });
    await db.query(
      'UPDATE orders SET gcash_proof=?,gcash_ref=?,gcash_status=?,gcash_reject_reason=NULL,status=? WHERE order_num=?',
      [gcash_proof, gcash_ref, 'pending_confirmation', 'pending', req.params.orderNum]
    );
    res.json({ status: 'success' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/products/restore-stock', requireAuth, async (req, res) => {
  const { cart } = req.body;
  if (!cart || !cart.length) return res.json({ status: 'ok' });
  try {
    for (const item of cart) {
      if (!item.id || !item.size) continue;
      const [rows] = await db.query('SELECT size_stock, stock FROM products WHERE id=?', [item.id]);
      ...
      if (ss[item.size] != null) {
        ...
        const newTotal = Object.values(ss).reduce((a,b)=>a+b, 0);
        await db.query('UPDATE products SET size_stock=?, stock=? WHERE id=?', [JSON.stringify(ss), newTotal, item.id]);
      }
    }
    res.json({ status: 'ok' });
  } catch (e) { res.status(500).json({ error: e.message }); }
...
app.put('/api/orders/:orderNum/return-request', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await db.query(
      'UPDATE orders SET status=?, return_reason=?, return_proof=? WHERE order_num=?',
      ['return_requested', reason||'', proof||null, req.params.orderNum]
    );
    res.json({ status: 'success' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
